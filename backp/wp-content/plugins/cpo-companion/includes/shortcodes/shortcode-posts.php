	...
	function cpo_shortcode_posts( $atts, $content = null ) {
		...
		if ( $query->have_posts() ) :
			...
			while ( $query->have_posts() ) :
				...
				$output .= '<div class="ctsc-column ctsc-col' . esc_attr( $columns ) . esc_attr( $col_last ) . '">';
				...
				$output .= '</div>';
				if ( 'list' != $style && 'none' != $excerpt ) {
					$output .= '<div class="ctsc-post-content">' . wp_kses_post( get_the_excerpt() ) . '</div>';
				}
				if ( '' != $readmore ) {
					$output .= '<a class="ctsc-post-readmore" href="' . get_permalink( get_the_ID() ) . '">' . esc_html( $readmore ) . '</a>';
				}
				$output .= '</div>';
				$output .= '</div>';
				$output .= '<div class="ctsc-clear"></div>';
