 ...
 *
 */
class Zels_Framework_Option_background extends Zels_Framework_Options {
  ...
  public function output() {
    ...
    $button_title   = ( isset( $button_title ) ) ? $button_title : __( 'Upload', ZELS_TEXTDOMAIN );
    $frame_title    = ( isset( $frame_title  ) ) ? $frame_title  : __( 'Upload', ZELS_TEXTDOMAIN );
    $insert_title   = ( isset( $insert_title ) ) ? $insert_title : __( 'Use Image', ZELS_TEXTDOMAIN );

    echo '<div class="zels-field-upload">';
    echo '<input type="text" name="'. $this->element_name( '[image]' ) .'" value="'. $this->value['image'] .'"'. $this->element_class() . $this->element_attributes() .'/>';
    echo '<a href="#" class="button zels-add" data-frame-title="'. $frame_title .'" data-upload-type="'. $upload_type .'" data-insert-title="'. $insert_title .'">'. $button_title .'</a>';
    echo '</div>';

    // background attributes
    echo '<fieldset>';
    echo zels_add_element( array(
        'pseudo'          => true,
        'type'            => 'select',
