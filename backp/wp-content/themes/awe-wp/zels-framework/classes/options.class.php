 ...
 *
 */
abstract class Zels_Framework_Options extends Zels_Framework_Abstract {
  ...
  public function element_after() {
    ...
    $out  = ( isset( $this->field['info'] ) ) ? '<p class="zels-text-desc">'. $this->field['info'] .'</p>' : '';
  ...
  public function element_debug() {
    ...
    if( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'ZELS_OPTIONS_DEBUG' ) && ZELS_OPTIONS_DEBUG ) ) {

      $value = $this->element_value();

      $out .= "<pre>";
      $out .= "<strong>". __( 'CONFIG', ZELS_TEXTDOMAIN ) .":</strong>";
      $out .= "\n";
      ob_start();
      var_export( $this->field );
      $out .= htmlspecialchars( ob_get_clean() );
      $out .= "\n\n";
      $out .= "<strong>". __( 'USAGE', ZELS_TEXTDOMAIN ) .":</strong>";
      $out .= "\n";
      $out .= ( isset( $this->field['id'] ) ) ? "zels_get_option( '". $this->field['id'] ."' );" : '';

      if( ! empty( $value ) ) {
        $out .= "\n\n";
        $out .= "<strong>". __( 'VALUE', ZELS_TEXTDOMAIN ) .":</strong>";
        $out .= "\n";
        ob_start();
        var_export( $value );
        $out .= htmlspecialchars( ob_get_clean() );
    ...
    if( is_array( $this->errors ) ) {
      foreach ( $this->errors as $key => $value ) {
        if( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
          $out .= '<p class="zels-text-warning">'. $value['message'] .'</p>';
        }
      }
    }
  ...
  public function element_help() {
    return ( isset( $this->field['help'] ) ) ? '<span class="zels-help" data-title="'. $this->field['help'] .'"><span class="fa fa-question-circle"></span></span>' : '';
  }
    ...
    if ( is_array( $this->multilang ) ) {
      ...
      foreach ( $this->multilang['languages'] as $key => $val ) {
        ...
        if( $key != $this->multilang['current'] ) {
          ...
          $class = 'Zels_Framework_Option_' . $this->field['type'];
      ...
      $out .= '</fieldset>';
      $out .= '<p class="zels-text-desc">'. sprintf( __( 'You are editing language: ( <strong>%s</strong> )', ZELS_TEXTDOMAIN ), $this->multilang['current'] ) .'</p>';
  ...
  public function element_multilang() {
    return ( isset( $this->field['multilang'] ) ) ? zels_language_defaults() : false;
  }

  public function get_settings_errors() {

    $errors = get_settings_errors( 'zels-framework-errors' );

    if ( $this->unique != ZELS_OPTION && empty( $errors ) ) {
